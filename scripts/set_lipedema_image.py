...
def run():
    ...
    # Find Article
    # Searching for title 'Lipedema' or similar
    article = Article.objects.filter(title__icontains="Lipedema").first()
    
    if not article:
        print("Error: Article 'Lipedema' not found.")
        return

    print(f"Found article: {article.title}")
    
    with open(image_path, 'rb') as f:
        # Saving as lipedema.png (normalized name)
        article.image.save('lipedema.png', File(f), save=True)
